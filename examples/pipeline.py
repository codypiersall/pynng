...
async def main():
    with pynng.Pull0() as pull, pynng.Push0() as push:
        
        n0 = await node0(pull)
        await curio.sleep(1)

        await node1(push, 'Hello, World!')
        await node1(push, 'Goodbye.')

        # another way to send
        async with curio.TaskGroup(wait=all) as g:
            for msg in ['A', 'B', 'C', 'D']:
                await g.spawn(node1, push, msg)
